!NCLFORTSTART
SUBROUTINE wrfcthcalc(prs, tk, qci, qcw, qvp, ght, ter, cth, pf, haveqci,&
    ...
    REAL(KIND=8), DIMENSION(ew,ns), INTENT(OUT) :: cth
...
!NCLEND
    ...
    ! LOCAL VARIABLES
    INTEGER i,j,k,ripk
    REAL(KIND=8) :: opdepthu, opdepthd, dp, arg1, fac, prscth, ratmix
    ...
    !$OMP prscth, dp, p1, p2, fac, arg1) SCHEDULE(runtime)
    DO j=1,ns
        DO i=1,ew
            opdepthd = 0.D0
            k = 0
            ...
            DO k=2,nz
                ...
                ELSE
                    ...
                    prscth = pf(i,j,k-1) + fac*(pf(i,j,k) - pf(i,j,k-1))
                    prscth = MIN(prs(i,j,1), MAX(prs(i,j,nz), prscth))
                    EXIT
                END IF
            END DO

            ! prscth should only be 0 if fill values are used
            IF (prscth .GT. -1) THEN
                DO k=2,nz
                    ripk = nz - k + 1
                    p1 = prs(i,j,ripk+1)
                    p2 = prs(i,j,ripk)
                    IF (prscth .GE. p1 .AND. prscth .LE. p2) THEN
                        fac = (prscth - p1)/(p2 - p1)
                        arg1 = fac*(ght(i,j,ripk) - ght(i,j,ripk+1))
                        cth(i,j) = ght(i,j,ripk+1) + arg1
                        EXIT
                    END IF
                END DO
            ELSE
